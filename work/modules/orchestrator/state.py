"""
Workflow state persistence — SQLite-backed task state for the 7-step pipeline.
"""
import sqlite3
import json
import os
...
class WorkflowState:
    ...
    def _load(self):
        ...
        self.total_steps = d.get("total_steps", 7)
    ...
    def save(self):
        ...
        conn.execute("""
            ...
            (id, user_id, story_type, gender, scene, status, current_step,
             ...
             video_url, video_error, error_message, updated_at)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
        """, (
