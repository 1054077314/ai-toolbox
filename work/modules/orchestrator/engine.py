"""
Workflow engine — runs the 6-step pipeline in a background thread.
"""
...
# ── Trigram similarity for historical dedup (threshold 60%) ──

def _trigram_similarity(a: str, b: str) -> float:
    """Trigram Jaccard similarity 0~1. Consistent with wenan/generator.py."""
...
_WORKFLOW_SANITIZE_MAP = [
    ...
    ("降血脂", "风味独特"),
    ("软化血管", "风味独特"),
    # ── Cleanup for polluted historical/generated phrases. Do not replace normal action verbs. ──
    ("S101在桌面上", "手持S101"),
...
def _sanitize_workflow_text(text: str) -> str:
    if not text:
        return ""
    result = str(text)
    for wrong, correct in _WORKFLOW_SANITIZE_MAP:
...
class WorkflowEngine:
    """Orchestrates the 7-step pipeline for one workflow."""
    ...
    def _check_similarity_retry(
        ...
        for attempt in range(max_retries + 1):
            ...
            if not history:
                return text, True
            max_sim = max(_trigram_similarity(text, h) for h in history)
            if max_sim <= 0.60:
                return text, True
    ...
    def _run_steps(self, start_step: int = 1):
        ...
        if start_step <= 6:
            ...
            try:
                ...
                if history:
                    max_sim = max(_trigram_similarity(st.video_prompt, h) for h in history)
                    if max_sim > 0.60:
                        logger.warning(
