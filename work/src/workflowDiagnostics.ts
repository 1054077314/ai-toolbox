...
export function buildWorkflowDiagnosticText({
  ...
  let stepLine: string;
  if (failedEvent) {
    stepLine = `当前步骤: ${failedEvent.step_index} / ${status?.total_steps ?? 6} ${failedEvent.step_name}`;
  } else {
    stepLine = `当前步骤: ${status?.step_index ?? 0} / ${status?.total_steps ?? 6} ${status?.current_step || ""}`;
  }
